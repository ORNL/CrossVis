...
//                axis.queryMeanPosition = axis.bottomPosition - (int) (normValue * axis.axisHeight);
            ...
            float lowValue = axis.column.getSummaryStats().getMean() - axis.column.getSummaryStats().getStandardDeviation();
...
//                }
                ...
                float normPos = (mouseOverAxisSelection.getMaxPosition() - mouseOverAxis.focusTop)
                        / (float) mouseOverAxis.focusHeight;
//                float normPos = (mouseOverAxisSelection.getMaxPosition() - mouseOverAxis.topPosition)
//                        / (float) mouseOverAxis.axisHeight;
                float maxValue = mouseOverAxis.column.getSummaryStats().getMax() -
...
//                        / (float) mouseOverAxis.axisHeight;
                normPos = (mouseOverAxisSelection.getMinPosition() - mouseOverAxis.focusTop)
                        / (float) mouseOverAxis.focusHeight;
                float minValue = mouseOverAxis.column.getSummaryStats().getMax() -
                        (normPos * (mouseOverAxis.column.getSummaryStats().getMax() - mouseOverAxis.column.getSummaryStats().getMin()));
...
//                float normPos = (queryMaxPosition - mouseOverAxis.topPosition) / (float) mouseOverAxis.axisHeight;
                float normPos = (queryMaxPosition - mouseOverAxis.focusTop) / (float) mouseOverAxis.focusHeight;
                float maxValue = mouseOverAxis.column.getSummaryStats().getMax() -
                        (normPos * (mouseOverAxis.column.getSummaryStats().getMax() - mouseOverAxis.column.getSummaryStats().getMin()));
...
//                normPos = (queryMinPosition - mouseOverAxis.topPosition) / (float) mouseOverAxis.axisHeight;
                normPos = (queryMinPosition - mouseOverAxis.focusTop) / (float) mouseOverAxis.focusHeight;
                float minValue = mouseOverAxis.column.getSummaryStats().getMax() -
                        (normPos * (mouseOverAxis.column.getSummaryStats().getMax() - mouseOverAxis.column.getSummaryStats().getMin()));
...
//						- (int) (normValue * axis.axisHeight);
	...
	public void arrangeColumnsByTypical(boolean useQueryLines) {
		...
		for (int i = 0; i < axisList.size(); i++) {
			...
			float typical = Float.NaN;
			if (this.getDispersionDisplayMode() == PCPanel.MEAN_DISPERSION_BOX_MODE) {
				if (useQueryLines) {
					typical = (float) axis.queryMeanPosition;
				} else {
					typical = (float) axis.meanPosition;
				}
			} else if (getDispersionDisplayMode() == MEDIAN_DISPERSION_BOX_MODE) {
				if (useQueryLines) {
					typical = (float) axis.queryMedianPosition;
				} else {
					typical = (float) axis.medianPosition;
				}
			}
			if (!Float.isNaN(typical)) {
	...
	public void arrangeColumnsByDispersion(boolean useQueryLines) {
		...
		for (int i = 0; i < axisList.size(); i++) {
			...
			float dispersion = Float.NaN;
			if (this.getDispersionDisplayMode() == PCPanel.MEAN_DISPERSION_BOX_MODE) {
				if (useQueryLines) {
					dispersion = (float) axis.queryStandardDeviationRangeRectangle.height;
				} else {
					dispersion = (float) axis.standardDeviationRangeRectangle.height;
				}
			} else if (getDispersionDisplayMode() == MEDIAN_DISPERSION_BOX_MODE) {
				if (useQueryLines) {
					dispersion = (float) axis.QueryIQRBoxRectangle.height;
				} else {
					dispersion = (float) axis.IQRBoxRectangle.height;
				}
			}
			if (!Float.isNaN(dispersion)) {
