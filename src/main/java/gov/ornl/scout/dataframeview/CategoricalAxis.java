...
public class CategoricalAxis extends Axis {
    ...
    public CategoricalAxis(DataFrameView dataFrameView, Column column, Orientation orientation) {
    ...
    public Set<String> getCategories() { return categoricalColumn().getCategories(); }
    ...
    protected void layout() {
        ...
        for (String category : categoricalColumn().getCategories()) {
            int categoryCount = categoricalColumn().getCategoryCount(category);

            Rectangle rectangle = null;
            if (orientation == Orientation.HORIZONTAL) {
                double categoryHeight = GraphicsUtil.mapValue(categoryCount, 0, categoricalColumn().getTotalCount(),
                        0, getAxisBarHeight());
                ...
                double categoryWidth = GraphicsUtil.mapValue(categoryCount, 0, categoricalColumn().getTotalCount(),
                        0, getAxisBarWidth());
...
//            rectangle.setOnMouseClicked(event -> { handleCategoryRectangleClicked(rectangle, category); });
            Tooltip.install(rectangle, new Tooltip(category + " : " + categoryCount + " of " +
                    categoricalColumn().getTotalCount() + " (" + percentageFormat.format((double)categoryCount / categoricalColumn().getTotalCount()) +
                    " of total)"));
