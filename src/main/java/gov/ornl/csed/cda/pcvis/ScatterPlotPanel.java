...
//			}
	...
	private double XPositionToValue(int x) {
		double normVal = (double) (x - plotRectangle.x) / (double) (plotRectangle.width);
		double val = xColumn.getSummaryStats().getMin() + (normVal * (xColumn.getSummaryStats().getMax() - xColumn.getSummaryStats().getMin()));
		return val;
	}

	private double YPositionToValue(int y) {
		double normVal = (double) (y - plotRectangle.y) / (double) (plotRectangle.height);
		double val = yColumn.getSummaryStats().getMax() - (normVal * (yColumn.getSummaryStats().getMax() - yColumn.getSummaryStats().getMin()));
		return val;
	}

	private void updateQueryPoints() {
		double minXQueryValue = XPositionToValue(dragRect.x);
		double maxXQueryValue = XPositionToValue(dragRect.x + dragRect.width);
		double maxYQueryValue = YPositionToValue(dragRect.y);
		double minYQueryValue = YPositionToValue(dragRect.y + dragRect.height);
