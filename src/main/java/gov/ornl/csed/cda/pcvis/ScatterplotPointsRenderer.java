...
public class ScatterplotPointsRenderer extends Renderer {
	...
	public void run() {
		...
		for (int ituple = 0; ituple < dataModel.getTupleCount(); ituple++) {
			...
			double yValue = currentTuple.getElement(yColumnIndex);
		...
		if (config.showRegressionLine && simpleRegression != null) {
			...
			int start_ix = toScreenX(startX, xColumn.getSummaryStats().getMin(), xColumn.getSummaryStats().getMax(), left, size);
			int start_iy = toScreenY(startY, yColumn.getSummaryStats().getMin(), yColumn.getSummaryStats().getMax(), top, size);
			int end_ix = toScreenX(endX, xColumn.getSummaryStats().getMin(), xColumn.getSummaryStats().getMax(), left, size);
			int end_iy = toScreenY(endY, yColumn.getSummaryStats().getMin(), yColumn.getSummaryStats().getMax(), top, size);
	...
	private int toScreenY(double value, double minValue, double maxValue, int offset, int plotHeight) {
		double normVal = 1. - ((value - minValue) / (maxValue - minValue));
		int y = offset + (int) (normVal * plotHeight);
		return y;
	}

	private int toScreenX(double value, double minValue, double maxValue, int offset, int plotWidth) {
		double normVal = (value - minValue) / (maxValue - minValue);
