...
public class PolylineRendererTimer extends AnimationTimer {
    ...
    private Color color;
    private ArrayBlockingQueue<TuplePolyline> polylineQueue;
    ...
    public PolylineRendererTimer (Canvas canvas, Collection<TuplePolyline> polylines, ArrayList<Axis> axisList,
    ...
    public void handle(long now) {
        ...
        for (int ipolyline = 0; ipolyline < maxPolylinesPerFrame; ipolyline++) {
            TuplePolyline polyline = polylineQueue.poll();
            if (polyline == null) {
                this.stop();
                break;
            } else {
                for (int i = 1; i < polyline.getTuple().getElementCount(); i++) {
